...
test path, including line-preserving `TEST_DRM_BUS_DISCOVERY` projections for
both detection executables. It also creates cscope, Ctags, and GNU Global
indexes, compiler dependency graphs, complexity reports, tool identities,
calibration results, and a verified SHA-256 manifest. The ignored generated
`include/version.h` stays outside the tracked denominator and enters only the
compiler include graph as a regenerated build dependency. The destination must
be empty.
